// Generated from runtime/alpha3/compat/anydoc/src/document-reader.ts; edit the TypeScript source.
...
export function createDocumentReader(convert = toMarkdownBytes) {
    ...
    async function read(fs, filePath, format, cwd, signal) {
        signal.throwIfAborted();
        const target = await fs.resolve(filePath, { cwd, signal });
        const info = await fs.stat(target, signal);
        if (!info || info.type !== 'file')
            throw Error('文件不存在或不是普通文件');
        if (info.size !== undefined && info.size > MAX_INPUT_BYTES)
            throw Error('文件超过 64 MiB 读取上限');
        ...
        const markdown = textOnly
            ...
            : dshNormalizeMarkdown(await convert(bytes, format ?? formatFromExtension(extension ?? '')));
        signal.throwIfAborted();
        if (!markdown.trim())
            throw Error('文档没有可读取的文本');
...
export function documentPage(markdown, offset = 0, limit = 16000) {
    if (!Number.isSafeInteger(offset) || offset < 0)
        throw Error('offset 必须是非负整数');
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 32000)
        throw Error('limit 必须是 1–32000 的整数');
    if (offset > markdown.length)
        throw Error('offset 超过文档字符总数');
    let end = Math.min(offset + limit, markdown.length);
    // Returned cursors must not cut a UTF-16 surrogate pair in two.
    if (end < markdown.length && /[\uD800-\uDBFF]/.test(markdown.charAt(end - 1)))
        end--;
    if (end === offset && offset < markdown.length)
        end = Math.min(offset + 2, markdown.length);
